...
//// downsampled by a certain declared variable
__global__ void kernel(unsigned char *input, 
                       int           *hist)
{
	register int location = blockIdx.x * blockDim.x+threadIdx.x;
	atomicAdd(&(hist[input[location*DOWN_SAMPLE+1]]),1);
}
...
{
	 register int location = threadIdx.x;
	 int diff = hist[255]-hist[0];
	 hist[location] = hist[location] - hist[0];
	 hist[location] = hist[location]*255/diff;	  
}
...
void update_val (unsigned char *data, 
		 int            size,
		 int           *hist)
{
...
void gpu_function(unsigned char *data,  
	...
	int           *hist; 
	int 	      *hist_cpu = new int[256];

	int gridXSize = 1 + (( width - 1) / TILE_SIZE);
	int XSize = gridXSize*TILE_SIZE;

	int size_refil = height*XSize;

	dim3 dimGridinit(size_refil/ (BLOCK_SIZE_1D*DOWN_SAMPLE));
	dim3 dimGrid((size_refil/ BLOCK_SIZE_1D)+1);
	dim3 dimBlock(BLOCK_SIZE_1D);


	// Allocate arrays in GPU memory
	checkCuda(cudaMalloc((void**)&input_gpu , size_refil*sizeof(unsigned char)));
        checkCuda(cudaMalloc((void**)&hist      , 256*sizeof(int)));
    
        checkCuda(cudaMemset(hist, 0 , 256*sizeof(int)));  // set histogram to 0
	...
	// begin timing
        ...
        // kernel calls
        kernel<<<dimGridinit, dimBlock>>>(input_gpu, 
                                          hist); 
	...
	update_val(data,size_refil,hist_cpu);
