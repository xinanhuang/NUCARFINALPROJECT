...
//// turn hist to shared memory	
//// also might wanna do it differently
__global__ void norm_cum( unsigned char *input,
			  int           *hist,
			  unsigned int   size)
{

         register int location = threadIdx.x;
	 register int dem = hist[0];

	 hist[location] = hist[location] - hist[0];
	 hist[location] = (long int)hist[location]*255/(size-dem);	  
}


//// version 2: slightly slower, prefix without shared memory 256*256 operation (possibly)
//__global__ void cum_calc( unsigned char *input,
//			  int           *hist,
//			  unsigned int   size)	
//{
...
//
//	  //write to result
//	  hist[location] = hist[location]*255/size;	  
//}
