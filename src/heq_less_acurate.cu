                       ...
                       int           *hist)
{
	...
	atomicAdd(&(hist[input[location*DOWN_SAMPLE]]),1);
}
...
//// more like inverse of alpha
//// avoid the issue of having to use 64bit long integer
__global__ void calc_alpha( int   *hist)
{
	hist[256] = (hist[255]-hist[0])/254;
}
...
__global__ void norm_cum( int   *hist)
{
	 ...
	 hist[location] = hist[location]/hist[256];	  
}
...
void gpu_function(unsigned char *data,  
	...
	dim3 dimGridinit(size/ (BLOCK_SIZE_1D*DOWN_SAMPLE));
	dim3 dimGrid((size/ BLOCK_SIZE_1D));
